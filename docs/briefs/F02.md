...
- [x] Frontend: login → TOTP → tenant switcher → logout works against the local stack; `npm run build` succeeds. *(owner pass, 2026-09-17)* *(Unticked in F02.1: verified only as the API flow through the Vite proxy. Closed by the owner's browser pass listed as the last acceptance criterion in `docs/briefs/F02.1.md`.)*
