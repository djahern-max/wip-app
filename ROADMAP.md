...
### F05.2 · Brand assets  ◐ (built 2026-09-26; the owner's browser pass on jobcost.dev and the swap test are open, see `docs/briefs/F05.2.md`)
An addition after F05.1; nothing moved. One image is the brand: `frontend/public/brand/logo.svg` (the hammer) plus `logo-mark.svg` for the 16 and 32 px favicons; `npm run brand` generates the favicons, touch and PWA icons, manifest and Open Graph image from them and `brand.json`, all committed; the app shell, sign-in page and the three static pages show the logo from the one file; `SITE_HOST` beside `PRODUCT_NAME` (D-09 untouched). No change to the backend, the CSP, the nginx site file or `deploy.sh`.
